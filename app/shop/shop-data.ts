...
function isSupportVeg(item: string) {
  ...
  return (
    lower.includes("onion") || lower.includes("leek") || lower.includes("garlic")
  );
}
...
function pickSupportVeg(dayIndex: number, family: DayFamily) {
  const supportFromFamily = family.supportItems.filter((item) => isSupportVeg(item));
...
function getSuggestedProducts(
  ...
  for (const [dayIndex, day] of DAYS.entries()) {
    ...
    const productNames = [
      ...family.shopBaseOptions.map((name) => ({ name, role: "base" as const })),
      ...family.shopBoostOptions.map((name) => ({ name, role: "boost" as const })),
    ];
...
function getFromBoxItems(week: Record<DayKey, GeneratedRecipe | null>) {
  ...
  const fromBox = ingredients.filter(
    (ingredient) =>
      HERO_VEG_POOL.some(
        (veg) => ingredient.includes(veg) || veg.includes(ingredient as (typeof HERO_VEG_POOL)[number]),
...
function getEverydayExtras(
  ...
  const suggestionTerms = suggestions.map((entry) => normalise(entry.product.name));
...
export default function PlannerPage() {
  ...
  const [week, setWeek] = useState<Record<DayKey, GeneratedRecipe | null>>(EMPTY_WEEK);
  ...
  const filledDays = useMemo(() => Object.values(week).filter(Boolean).length, [week]);
          ...
          <div className="grid gap-4 lg:grid-cols-[1.12fr_0.88fr]">
            <div className="rounded-[28px] border border-[rgba(221,212,200,0.95)] bg-[rgba(247,242,235,0.84)] p-5 shadow-[0_12px_30px_rgba(36,51,40,0.06)] md:p-7">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                >
                  {isPlanningWeek ? "Planning your week..." : "Start planning your week"}
                </button>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-[#5f675c]">
                Each day starts from a different veg identity, but the bases stay
                flexible so the plan feels helpful, not rigid.
              </p>
            </div>
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="font-medium text-[#243328]">{day.label}</h3>
                          ...
                          <div className="mt-4 rounded-[18px] border border-[#e6ddd2] bg-[rgba(249,246,241,0.78)] p-4">
                            ...
                            <ol className="mt-3 space-y-3 text-sm leading-6 text-[#243328]">
                              {recipe.steps.map((step, index) => (
                                <li key={`${day.key}-${index}`} className="flex gap-3">
          ...
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-4 lg:grid-cols-[0.98fr_1.02fr]">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {fromBoxItems.length > 0 ? (
                        ...
                        <p className="text-sm text-[#5f675c]">
                          Plan a few days and your box staples will start to show here.
                        </p>
                      )}
                    </div>
                  </div>
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-1 text-xs text-[#6b776c]">
                                Used across {formatNaturalList(entry.matchedDays)}
                              </p>
                            </div>
                            ...
                            <div className="flex flex-col items-end gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => addSuggestedItem(entry.product.name)}
              ...
              <article className="rounded-[24px] border border-[rgba(221,212,200,0.95)] bg-[rgba(247,242,235,0.86)] p-5 shadow-[0_10px_24px_rgba(36,51,40,0.05)]">
                ...
                <h2 className="mt-1 font-serif text-[1.45rem] leading-tight text-[#243328]">
                  Flexible enough to feel real, commercial enough to support the week
                </h2>
                <p className="mt-2 text-sm leading-6 text-[#5f675c]">
